...
// Fetch real UV index from OpenWeatherMap for given coordinates
async function fetchRealUV(lat: number, lng: number): Promise<number> {
  ...
  try {
    ...
    console.error('Failed to fetch UV from OpenWeatherMap:', err);
    return 0;
  }
}
...
export function UVMap({ onLocationSelect, currentUv, initialLocation }: UVMapProps) {
  ...
  const processLocation = async (lat: number, lng: number) => {
    ...
    // Fetch real UV and reverse geocode in parallel
    const [uv, locationName] = await Promise.all([
      fetchRealUV(lat, lng),
  ...
  async function reverseGeocode(lat: number, lng: number): Promise<string> {
    try {
      // Use Nominatim (free, no key needed) as primary reverse geocoder
      const res = await fetch(
      ...
      // Fall through to geocode.maps.co if Nominatim fails
      if (GEOCODE_API_KEY) {
        try {
          const res = await fetch(
  ...
  async function forwardGeocode(query: string): Promise<{ lat: number; lon: number } | null> {
    try {
      ...
      // Fall through to geocode.maps.co
      if (GEOCODE_API_KEY) {
        try {
          const res = await fetch(
          ...
          if (validResult) {
            return { lat: parseFloat(validResult.lat), lon: parseFloat(validResult.lon) };
          }
        } catch (err) {
  ...
  useEffect(() => {
    ...
    // Click anywhere on map to get real UV for that point
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result && isWithinAustralia(result.lat, result.lon)) {
        ...
        alert('Location not found or outside Australia. Please try a different search term.');
      }
    } catch (err) {
  ...
  return (
    <div className="w-full h-full rounded-xl overflow-hidden bg-gray-100 relative z-0 min-h-[400px]">
        ...
        <div className="absolute top-4 left-4 right-4 z-[400] flex gap-2">
          ...
          <div className="ml-auto">
            ...
            >
              <LocateFixed
                size={20}
                className={isLocating ? 'animate-pulse text-[#FF6900]' : 'text-[#4a5565]'}
              />
      ...
      <div className="absolute bottom-4 left-4 right-4 bg-white/90 backdrop-blur-sm border border-black/10 text-sm text-center py-2 px-4 rounded-lg shadow-sm z-[400] pointer-events-none">
        Click on the map or search to get real-time UV details
      </div>
    </div>
  );
}
